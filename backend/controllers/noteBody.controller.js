...
export const  editNote= async (req,res)=>{
    const {notebodyId}=req.params
    const change=req.body

    try{
        const edited=await NoteBody.findByIdAndUpdate(notebodyId,change)
        res.status(200).json({sucess:true,message:"updated file",data:edited})
    }catch{
        res.status(500)
    }
}